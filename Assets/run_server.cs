	...
	void Update ()
    {
        ...
        if (message != "")
        {
            ...
            if (my_info.c_type == "GRID")
            {
                ...
                arr2 = new float[my_info.time_dim, reso];
                for (int i = 0; i < (my_info.time_dim); i++)
                {
                    for (int j = 0; j < reso; j++)
                    {
                        arr2[i, j] = temp[i * reso + j];
                    }
                }
            ...
            else if (my_info.c_type == "SCATTER")
            {
                ...
                arr3 = new float[my_info.y_dim, 3];
            ...
            {
                Debug.Log("Heat Map");
                arr4 = new float[my_info.x_dim, my_info.y_dim, my_info.z_dim];
                for (int i = 0; i < my_info.x_dim; i++)
                {
                    for (int j = 0; j < my_info.y_dim; j++)
                    {
                        ...
                        {
                            arr4[i, j, k] = temp[i * my_info.y_dim * my_info.z_dim + j * my_info.z_dim + k];
                        }
                    }
                }
                mode = 4;
